...
# Convert slug to plugin name (capitalize each word) - macOS compatible
PLUGIN_NAME=$(echo "$PLUGIN_SLUG" | awk -F'-' '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) substr($i,2); print}' OFS=' ')

# Convert slug to namespace (PascalCase with backslashes) - macOS compatible
NAMESPACE=$(echo "$PLUGIN_SLUG" | awk -F'-' '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) substr($i,2); print}' OFS='\\')

# Try to get git user info, fallback to defaults
...
# Extract vendor name from email (e.g., "user@example.com" -> "user")
# But make it slug-friendly (lowercase, no dots)
VENDOR_NAME=$(echo "$GIT_EMAIL" | cut -d'@' -f1 | tr '[:upper:]' '[:lower:]' | tr '.' '-')
...
if [ -f "composer.json" ]; then
    sed -i '' "s/\"name\": \"devkabir\/wordpress-plugin\"/\"name\": \"$VENDOR_NAME\/$PLUGIN_SLUG\"/g" composer.json
...
if [ -f "includes/autoload.php" ]; then
    ...
    sed -i '' "s/YourPlugin\\\\\\\\/$NAMESPACE\\\\/g" includes/autoload.php
