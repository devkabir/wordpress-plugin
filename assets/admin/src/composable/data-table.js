      ...
      columns.forEach((col) => {
        if (col.key !== column.key) {
          result +=
            ...
            col.label +
            "</td><td>" +
            generateHtml(col, row[col.key], col.text) +
            "</td></tr>";
        }
      });
      result += "</table>";
...
export function useSearch(data, props) {
  ...
  const anySearchable = computed(() =>
    props.columns.flatMap((obj) => (obj.search === true ? [obj.key] : []))
  );
...
export function usePagination(data, props) {
  ...
  let pageNumbers = computed(() => {
    ...
    return [
      ...createRangeArray(currentPage.value, currentPage.value + 2),
      "...",
      ...createRangeArray(totalPages.value - 2, totalPages.value),
    ];
  });

  // Define methods to navigate between pages
  const previousPage = () =>
    currentPage.value === 1 ? null : currentPage.value--;
  const nextPage = () =>
    currentPage.value === totalPages.value ? null : currentPage.value++;
...
export function useBulkAction(data, columns) {
  ...
  const selected = () =>
    (allSelected.value = selectedRows.value.length === data.value.length);
