<script setup>
import {useBulkAction, useDataTable, usePagination, useSearch, useSort} from '@/composable/data-table.js';
...
<template>
  <div>
    <div v-if="anySearchable.length" class="flex items-baseline justify-end mb-2">
      <div>
        ...
        <tr>
          <th v-if="selectKey" class="relative w-12 px-6 sm:w-16 sm:px-8" scope="col">
            <input v-model="allSelected"
                   ...
                   type="checkbox" @change="selectAll">
          </th>
          <th v-for="(header, headerIndex) in columns" :key="headerIndex" :class="generateClass(header)" scope="col">
              ...
              <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                ...
                <div>
                  <nav aria-label="Pagination" class="isolate inline-flex -space-x-px rounded-md shadow-sm">
                    <button :disabled="currentPage === 1" class="button-previous" type="button" @click="previousPage">
                      <span class="sr-only">Previous</span>
                      <ChevronLeftIcon class="w-5 h-5"/>
                    </button>
                    <button v-for="page in pageNumbers" :key="page"
                            :class="{ 'button-page-current':page === currentPage }"
                            aria-current="page" class="button-page" type="button" @click="goTo(page)"
                            v-text="page"></button>
                    <button :disabled="currentPage === totalPages" class="button-next" type="button" @click="nextPage">
