...
const { isLoading, data, columns, generateClass, formatColumnData } =
  useDataTable(props);

const { sortData, sortedColumns } = useSort(data);

const { search, anySearchable, searchResults } = useSearch(data, props);
...
const { selectKey, selected, allSelected, selectAll, selectedRows, deleteAll } =
  useBulkAction(paginatedData, props.columns);
</script>
<template>
  <div>
    <div
      v-if="anySearchable.length"
      class="flex items-baseline justify-end mb-2"
    >
      <div>
        <input
          v-model="search"
    ...
    >
      ...
      <div v-if="selectKey && selectedRows.length" class="bulk-wrapper">
        <button class="px-10 button-light" type="button" @click="deleteAll">
          Delete all
        </button>
      </div>
      <table
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
          ...
          >
            <td v-if="selectKey" class="relative w-12 px-6 sm:w-16 sm:px-8">
              ...
              <div
                v-if="selectedRows.find((id) => id === row[selectKey])"
            ...
            <td
              ...
              :key="columnIndex"
              :class="generateClass(column)"
              v-html="formatColumnData(row, columns, column)"
            ></td>
          </tr>
        </tbody>
        <tfoot>
          <tr>
            <td :colspan="props.columns.length + 1">
              <div
                class="flex items-center justify-between bg-gray-50 px-4 py-3 sm:px-6"
              >
                  ...
                  <div>
                    ...
                    >
                      ...
                      <button
                        ...
                        :key="page"
                        :class="{ 'button-page-current': page === currentPage }"
                        aria-current="page"
                        class="button-page"
                        type="button"
                        @click="goTo(page)"
      ...
      <div v-if="!isLoading && paginatedData.length === 0" class="placeholder">
